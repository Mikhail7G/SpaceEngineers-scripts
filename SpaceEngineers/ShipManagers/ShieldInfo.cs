...
namespace SpaceEngineers.ShipManagers.ShieldInfo
{
    public sealed class Program : MyGridProgram
    {
        ...
        public void Update()
        {
            if (shieldWrapper.GridHasShield(Me.CubeGrid))
            {
                Echo("True");
        ...
        {
            if (display == null || display.Closed)
                return;

            display?.WriteText("", false);
            display?.WriteText($"Persent: {shieldWrapper.GetShieldPercent()} %" +
                               $"\nHP Cap: {shieldWrapper.GetMaxHpCap()}" +
                               $"\nChargeRate: {shieldWrapper.GetChargeRate()}" +
                               $"\nHeat: {shieldWrapper.GetShieldHeat()}" +
                               $"\nPower: {shieldWrapper.GetPowerUsed()}" +
                               $"\nSta: {shieldWrapper.ShieldStatus()}", true);
